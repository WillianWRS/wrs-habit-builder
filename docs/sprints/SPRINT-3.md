...
> Tema: **Métricas do hábito + adesão visível** — conteúdo replanejado da antiga Sprint 3.

| ID | Tarefa | Descrição (1 frase) |
|----|--------|---------------------|
| S4-01 | Utils de adesão | `computeAdherence` + `formatAdherenceDisplay` com regra progressiva (RN-04) |
| S4-02 | Rota detalhe/progresso individual | `/habits/:id` ou `/habits/:id/progress` — shell + header + link editar |
| S4-03 | Heatmap individual | Grid 30–66d por hábito no detalhe |
| S4-04 | Streak + freeze no detalhe | Métricas consolidadas + inventário de escudos |
| S4-05 | Adesão 7d/30d na UI | Labels adaptativos; nunca `30d` no dia 2 |
| S4-06 | Navegação lista → detalhe | Chevron / área clicável no card |
| S4-07 | Chips de adesão | Lista `/habits` + resumo em `/progress` |

**Fora de escopo Sprint 4:** PWA, templates onboarding, monetização, sync.

**Pendência pré-alfa (não é sprint de dev):** decisão de nome oficial do produto.
